...
def held_record_leak(repo: Path) -> None:
    path, data, metadata = records(repo)
    register = json.loads((repo / metadata["under_review_file"]).read_text(encoding="utf-8"))
    data.append(register["entries"][0]["record"])
    write_records(path, data)
...
def main() -> int:
    ...
    if original_metadata.get("under_review_file"):
        specifications.insert(-1, ("held_record_leaks_into_public_catalog", held_record_leak, False, []))
