...
def schema_and_invariants(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    under_review_file = metadata.get("under_review_file")
    under_review_schema_file = metadata.get("under_review_schema_file")
    if under_review_file or under_review_schema_file:
        if not under_review_file or not under_review_schema_file:
            failures.append("under-review metadata requires both file and schema file")
        else:
            register_path = ROOT / under_review_file
            register_schema_path = ROOT / under_review_schema_file
            if not register_path.is_file() or not register_schema_path.is_file():
                failures.append("under-review register or schema file is missing")
            else:
                register = json.loads(register_path.read_text(encoding="utf-8"))
                register_schema = json.loads(register_schema_path.read_text(encoding="utf-8"))
                ...
                for error in sorted(review_validator.iter_errors(register), key=lambda item: list(item.absolute_path)):
                    locator = "/".join(str(part) for part in error.absolute_path) or "under-review register"
                    failures.append(f"{under_review_file} {locator}: {error.message}")
                entries = register.get("entries", [])
                if register.get("record_count") != len(entries):
                    failures.append("under-review record_count does not match entries")
                public_ids = {record.get("id") for record in records}
                for entry in entries:
                    record = entry.get("record", {})
                    record_id = record.get("id")
                    if record_id in public_ids:
                        failures.append(f"{record_id}: appears in both public and under-review data")
                    if record_id in under_review_ids:
                        failures.append(f"{record_id}: duplicate under-review stable ID")
                    under_review_ids.add(record_id)
                    if record.get("audit_state") != "hold":
                        failures.append(f"{record_id}: under-review record must retain audit_state=hold")
    ...
    return {
        ...
        "failures": failures,
        "warnings": warnings,
        "under_review_records": len(under_review_ids),
    }
